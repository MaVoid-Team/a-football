class CreateSegmentsAndMessageTemplates < ActiveRecord::Migration[8.0]
  ...
  def seed_default_segments
    ...
    execute <<~SQL.squish
      ...
      VALUES
        ('active', '{"operator":"all","rules":[{"field":"last_activity_days","op":"lte","value":7}]}'::jsonb, true, NULL, #{quoted_now}, #{quoted_now}),
        ('inactive', '{"operator":"all","rules":[{"field":"last_activity_days","op":"gte","value":30}]}'::jsonb, true, NULL, #{quoted_now}, #{quoted_now}),
        ('frequent', '{"operator":"all","rules":[{"field":"total_bookings","op":"gte","value":3}]}'::jsonb, true, NULL, #{quoted_now}, #{quoted_now}),
        ('tournament_players', '{"operator":"all","rules":[{"field":"total_tournaments","op":"gte","value":1}]}'::jsonb, true, NULL, #{quoted_now}, #{quoted_now})
    SQL
  end
end
